    ...
    public void lendItem(Patron patron, Item item) {
        if (items.contains(item)) {
            if (!item.isBorrowed() && patrons.contains(patron)) {
                ...
                System.out.println("Предмет уже взят на время: " + item.getTitle());
            }
        } else {
            System.out.println("Предмет не найден в библиотеке: " + item.getTitle());
        }
    }
    ...
    public void returnItem(Patron patron, Item item) {
        if (items.contains(item)) {
            if (item.isBorrowed() && patrons.contains(patron)) {
                ...
                System.out.println("Предмет не был одолжен: " + item.getTitle());
            }
        } else {
            System.out.println("Предмет не найден в библиотеке: " + item.getTitle());
        }
